...
class MySQLHelper {
    ...
    public function query($sql) {
        ...
        // 检查是否需要添加到事务队列当中
        if (!$this->_transStarted && App::isNeedJoinInTrans()) {
            $this->start();
    ...
    public function error($where, $sql = '') {
        throw new UserException(UserException::ERROR_MYSQL, "Where:%s\nSQL:%s\nError:%s\nErrno:%d", $where, $sql, mysql_error($this->_linkId), mysql_errno($this->_linkId));
    }
}